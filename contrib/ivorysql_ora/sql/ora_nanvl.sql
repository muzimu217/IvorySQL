...
-- * expr1 NaN and expr2 NULL -> result is NULL (expr2 is the result).
--
...
SELECT NANVL(CAST('NAN' AS BINARY_FLOAT), CAST(NULL AS BINARY_FLOAT)) FROM DUAL;
SELECT NANVL(CAST(1.5 AS BINARY_DOUBLE), CAST(NULL AS BINARY_DOUBLE)) FROM DUAL;
SELECT NANVL(CAST('NAN' AS BINARY_DOUBLE), CAST(NULL AS BINARY_DOUBLE)) FROM DUAL;
